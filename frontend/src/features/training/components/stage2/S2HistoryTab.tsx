...
import { listStage2History } from '../../api/stage2Api';
...
export default function S2HistoryTab() {
  const { history, setHistory, historyLoading, setHistoryLoading } = useStage2Store();

  const loadHistory = async () => {
    setHistoryLoading(true);
    try {
      const res = await listStage2History();
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 700 }}>Stage 2 Training History</h3>
          <span style={{
      ...
      {historyLoading && history.length === 0 ? (
        ...
        <div style={{
          ...
          <div style={{ fontWeight: 600, marginBottom: '6px' }}>No Stage 2 training runs yet</div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 4px' }}>
            <thead>
              <tr>
                {[
                  'Run ID', 'Variant Created', 'Meeting', 'Groups', 'Batch Size',
                  'Context', 'Optimizer', 'Model', 'Score',
                ...
                return (
                  <tr key={entry.run_id} style={{
                    ...
                    <td style={{ padding: '0.6rem 0.75rem', fontSize: '0.8rem', textAlign: 'center' }}>
                      <span style={{ fontWeight: 600 }}>{entry.training_group_count}</span>
                      <span style={{ color: 'hsl(var(--muted-foreground))', fontSize: '0.72rem' }}> / {entry.validation_group_count}</span>
                    </td>
                    <td style={{ padding: '0.6rem 0.75rem', fontSize: '0.78rem', textAlign: 'center' }}>
                      {entry.points_per_group || 5} pts
                    </td>
                    <td style={{ padding: '0.6rem 0.75rem', fontSize: '0.75rem', textAlign: 'center' }}>
                      {entry.context_retrieval ? '✓ On' : 'Off'}
                    </td>
                    ...
                    <td style={{ padding: '0.6rem 0.75rem', textAlign: 'center' }}>
                      <span style={{
                        ...
                        color: entry.final_score >= 80 ? 'hsl(142 71% 45%)' : entry.final_score >= 60 ? 'hsl(45 93% 47%)' : 'hsl(0 75% 55%)'
                      }}>
                        {entry.final_score}%
                      </span>
                    </td>
